...
def get_clinical_label(clinical, slides_path, status=None):
    clinical = clinical.rename(columns={
        ...
        "DSS_MONTHS": "Months of disease-specific survival",
        "DSS_STATUS": "Disease-specific Survival status",
        "OS_MONTHS": "Overall Survival (Months)",
        "OS_STATUS": "Overall Survival Status",
        "PFS_MONTHS": "Progress Free Survival (Months)",
        "PFS_STATUS": "Progression Free Status",
    })
    ...
    if case_dict:
        ...
        print(f"No WSI files found under {slides_path}; keeping clinical labels with case-id placeholders.")
        clinical["wsi"] = clinical["case id"]
